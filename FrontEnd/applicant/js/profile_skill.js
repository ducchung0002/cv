...
async function reload_skill_dropdown(applicant_skill_id, def) {
    ...
    skill_of_type.forEach((skill) => {
        let option = document.createElement("option");
        option.classList.add();
        option.value = skill["id"];
        option.textContent = skill["name"];
        option.selected = skill["id"] === def;
 ...
 */
async function load_skill_type_dropdown(applicant_skill_id, def) {
    let skill_type_dropdown = document.getElementById(`skill_type_dd_${applicant_skill_id}`);
...
async function load_skill_dropdown(applicant_skill_id, def) {
    let skill_type_dropdown = document.getElementById(`skill_type_dd_${applicant_skill_id}`);
    let skill_dropdown = document.getElementById(`skill_dd_${applicant_skill_id}`);
    let selected_skill_type_id = skill_type_dropdown.value;
    skill_type_dict[selected_skill_type_id]["skill"].forEach((skill) => {
        let option = document.createElement("option");
        option.classList.add();
        option.value = skill["id"];
        option.textContent = skill["name"];
...
async function load_experience_dropdown(applicant_skill_id, def) {
 ...
 */
async function update_applicant_skill(applicant_skill_id) {
    let skill_dropdown = document.getElementById(`skill_dd_${applicant_skill_id}`);
    let experience_dropdown = document.getElementById(`experience_dd_${applicant_skill_id}`);
...
async function delete_applicant_skill(applicant_skill_id) {
...
async function trigger_update_button(applicant_skill_id) {
    let update_btn = document.getElementById(`update_skill_button_${applicant_skill_id}`);
 ...
 */
async function insert_applicant_skill() {
    let skill_type_id;
    let skill_id;
    let experience_id;
    try {
...
async function load_skill_information() {
    axios.get("http://127.0.0.1:5000/applicant/skill", {
        ...
        /************************/
        let skill_div = document.getElementById("skill_container");
        applicant_skill.sort(function (a, b) {
            return a["skill_type_id"] - b["skill_type_id"];
        });
